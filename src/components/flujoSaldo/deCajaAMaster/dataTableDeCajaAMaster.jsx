...
const VISIBLE_FIELDS = ['nameCaja', 'phoneCaja', 'Fecha', 'quantSolde', 'quantSoldeCaja','cantidad_resultante', 'nameMaster', 'phoneMaster', 'quantSoldeCount', 'fechaA', 'createdAt', 'Acciones'];
...
export default function DataTableDeCajaAMaster() {
    ...
    const GetEnviosCajaMasters = async (userId) => {
        ...
        try {
            const res = await AxiosConfigsToken.get(`/obtener_de_caja_a_master_id_caja/${userId}`)
            ...
            >
                ...
                <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                    <Box sx={{ width: '100%' }}>
                        ...
                        <LoadingButton
                            onClick={() => { Buscar('filtrar_de_caja_a_master_caja_master') }}
                            loading={buscar}
                            variant="contained"
                            color="primary"
                            sx={{ width: "100%" }}
